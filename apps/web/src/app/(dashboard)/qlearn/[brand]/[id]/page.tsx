...
const Page = () => {
    const menuItems = ["canvas", "both", "learn"];
    ...
    const data = AI.find(category => category.category === "QLearn")
        ...
        <main className="w-95 relative mx-auto flex flex-col flex-grow gap-5 py-3">
            ...
            >
                ...
                <ResizablePanel
                    ...
                    minSize={0}
                >
                   <div className={`ml-auto w-[99%] h-full backdrop-blur-3xl backdrop-filter rounded-3xl overflow-hidden flex items-center justify-center ${active === 'learn' ? 'w-full h-full' : ''}`}>
                        {data ? <AiContentLoader component={data} /> : null}
                    </div>
                </ResizablePanel>
            </ResizablePanelGroup>
        </main>
