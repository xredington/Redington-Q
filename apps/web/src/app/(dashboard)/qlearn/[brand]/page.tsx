...
const Page = () => {
    ...
    const brandCategory = AI.find(item => item.category === "QLearn")
    ...
    if (!userHasAccess) {
        return (
            <main className="flex-grow mt-10 relative w-95 mx-auto  flex items-center justify-center pb-6">
                <ErrorComponent
            ...
            <section className={"grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-4xl mx-auto auto-rows-[18rem] lg:auto-rows-[13.5rem]"}>
                {brandCategory.components.map((item) => (
                    <Link key={item.id} href={`${pathname}/${item.id}`} className={`${item.className} row-span-1`}>
                        <div className={`h-full w-full relative group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none dark:bg-black dark:border-white/[0.2] ${item.id === 'course-content-ai' || item.id === 'interactive-demo' ? 'bg-green-700/60' : 'bg-[#F3F4ED]'} border border-transparent justify-between flex flex-col space-y-4 rounded-3xl overflow-hidden`}>
                            {item.id === 'course-content-ai' || item.id === 'interactive-demo' ? (
                            ...
                            <div className="px-3 w-full h-full flex flex-col items-start justify-center gap-3 z-20 sticky">
                                <p className={` ${item.id === 'course-content-ai' || item.id === 'interactive-demo' ? 'text-slate-50' : 'text-green-600'}`}>
                                    {item.icon && <item.icon />}
                                </p>
                                <div className="flex justify-between items-center w-full">
                                    <h3 className={`text-xl sm:text-2xl font-bold ${item.id === 'course-content-ai' || item.id === 'interactive-demo' ? 'text-slate-50' : 'text-card-foreground'}`}>
                                        {item.title}
                                    </h3>
                                </div>
                            </div>
